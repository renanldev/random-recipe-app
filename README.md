# random-recipe-app
Aplicativo web responsivo que gera receitas aleatórias de diferentes partes do mundo e ensina o usuário a prepará-las rapidamente.

🍽️ Getta Recipe

Aplicação web que sorteia receitas aleatórias de diferentes partes do mundo e mostra como prepará-las de forma rápida e simples.

O objetivo do projeto é permitir que o usuário descubra novas receitas com apenas um clique.

---

🚀 Funcionalidades

- 🎲 Sorteio aleatório de receitas
- 🌎 Receitas de diferentes países
- 🖼️ Exibição da imagem do prato
- 📋 Instruções de preparo
- 📱 Layout responsivo
- ⚡ Interface simples e rápida

---

🛠️ Tecnologias utilizadas

- **HTML5**
- **CSS3**
- **JavaScript**
- API de receitas: https://www.themealdb.com

---

📸 Demonstração

![preview do projeto](<img width="1893" height="920" alt="image" src="https://github.com/user-attachments/assets/90925e51-4843-4806-ac29-ad90f86bd3de" />
)

---

📂 Como executar o projeto

1. Clone o repositório

```
git clone https://github.com/seu-usuario/getta-recipe.git
```

2. Abra a pasta do projeto

3. Execute o arquivo **index.html** no navegador

---

💡 Objetivo do projeto

Este projeto foi desenvolvido com fins de aprendizado, praticando:

- consumo de **APIs**
- manipulação do **DOM**
- criação de **interfaces responsivas**

---

📌 Melhorias futuras

- tradução automática das receitas
- animação ao exibir nova receita
- botão para sortear outra receita
- filtro por categoria ou país
- modo escuro

---

## 👨‍💻 Autor

Projeto desenvolvido por **Renan Souza**.
